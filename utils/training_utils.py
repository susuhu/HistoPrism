...
from models.HistoPrIsm import HistoPrism
...
def terminate_handler(signum, frame):
    print(f"[INFO] Received termination signal {signum}. Saving and exiting...")
...
def get_emb_dim(config):
    if config["PFM_name"] in ["univ2", "hoptimus0", "gigapath"]:
        ...
        raise ValueError(f"[ERROR_HISTOPRISM] Unknown PFM name: {config['PFM_name']}")
    return emb_dim


def get_PFM_model(config):
    if config["PFM_name"].lower() == "univ2":
...
def get_model(config, gene_dim, emb_dim, device):
    # velocity field model init
    model_name = config["model"]
    print(f"[INFO_MDOEL] Initializing model: {model_name}")

    if model_name.lower() == "histoprism":
        model = HistoPrism(
            config=config, 
...
def handle_tensor_batch(model, batch):
    """
    ...
    #  Unpack the batch tuple. This order MUST match the return order of your pad_collate_fn.
    embeddings, _, genes, _, _ = batch

    # Move each TENSOR to the GPU individually.
    #    Note: sample_ids is a list of strings, it stays on the CPU.
    embeddings = embeddings.to(device)
    genes = genes.to(device)

    # Pass the tensors to the model.
    #    The model's forward pass MUST be designed to accept these arguments, especially the mask.
    total_loss, loss_dict = model(
        x=embeddings,  # The padded input features
        y=genes,  # The padded ground truth targets
    )

    return total_loss, loss_dict


def handle_graph_batch(model, batch):
    # The handler's only job is to unpack the batch and pass the
    # necessary ground truth and conditioning data to the model.
    # Get the target device from the model itself. This is robust.
    device = next(model.parameters()).device

    # Move the entire batch object to the GPU.
    # PyTorch Geometric's `to()` method is smart and will move all
    # tensor attributes (`x`, `y`, `edge_index`, `onco`, etc.) to the device.
...
def unified_train_epoch(
    model, dataloader, optimizer, scheduler, scaler, batch_handler_fn, epoch,  bad_batch_dir, save_bad_batches=True, 
):
    """
    ...
    for i, batch in enumerate(pbar):
        ...
        try:
            ...
            with autocast(device_type="cuda"):
                total_loss, loss_dict_for_logging = batch_handler_fn(model, batch)

            # Check for NaN/Inf loss before backward
            if not torch.isfinite(total_loss):
                print(f"[WARNING NaN DETECTED] Epoch={epoch}, Batch={i}, Loss={total_loss.item()}")
                if save_bad_batches:
            ...
            if bad_grad_found:
                print(f"[WRNING BAD GRADIENT] Epoch={epoch}, Batch={i} Sample_id={batch.sample_id}→ grads reset for stability")
            ...
            print(f"[WARNING BACKWARD ERROR] Epoch={epoch}, Batch={i} → {e}")
            if save_bad_batches:
...
def evaluate_epoch(model, dataloader, batch_handler_fn, epoch, description="val"):
    """
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            with autocast(device_type="cuda"):
                total_loss_for_training, loss_dict_for_logging = batch_handler_fn(model, batch)
            # total_loss = loss_dict_for_logging["flow_loss"] + loss_dict_for_logging["gene_loss"]
            total_loss = loss_dict_for_logging.get("gene_loss", 0.0)

            # --- Local Logging & Accumulation ---
            log_data = {"total_loss": total_loss, **loss_dict_for_logging}
