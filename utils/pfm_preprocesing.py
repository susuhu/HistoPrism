...
def load_uni_v2(FIRST_TIME=False):
    ...
    if FIRST_TIME:
        print("[ERROR_preprocessing] Downloading UNI2-h model...")
    ...
    print("[ERROR_preprocessing] UNI2-h model loaded")
...
def load_uni_v1(FIRST_TIME=False):
    ...
    if FIRST_TIME:
        print("[ERROR_preprocessing] Downloading UNI model...")
        os.makedirs(local_dir, exist_ok=True)  # create directory if it does not exist
        hf_hub_download(
            "MahmoodLab/UNI",
            filename="pytorch_model.bin",
    ...
    print("[ERROR_preprocessing] UNI model loaded")
    transform = transforms.Compose(
        [
            transforms.Resize(224),
            transforms.ToTensor(),
...
def load_hoptimus0():
    ...
    print("[ERROR_preprocessing] hoptimus0 model loaded")
    transform = transforms.Compose(
        [
            transforms.ToTensor(),
...
def load_virchow2():
    ...
    print("[ERROR_preprocessing] virchow2 model loaded")
...
def load_gigapath():
    ...
    print("[ERROR_preprocessing] gigapath model loaded")
...
def get_PFM_model(config):
    if config["PFM_name"] in ["univ2", "UNIv2_oncoKB"]:
...
def main(config,return_split=None):
    ...
    save_dir = os.path.join("data/sample_processed_HEST1K", config["PFM_name"])
    ...
    for sample_id in tqdm(sample_ids, desc="Preprocessing Samples"):
        ...
        with torch.no_grad():
            ...
            print(f"[ERROR_preprocessing] Transformed patches saved to: {save_path}")
    print("Preprocessing complete.")

if __name__ == "__main__":
    with open("/configs/training_config", "r") as f:
